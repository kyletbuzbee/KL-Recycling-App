...
import 'package:kl_recycling_app/core/utils/error_messages.dart';
...
class LoyaltyService {
  ...
  Future<LoyaltyProfile> getLoyaltyProfile(String userId) async {
    try {
      final data = await _firebaseService.firestore.collection('loyalty_profiles').doc(userId).get();
      return LoyaltyProfile.fromMap(data.data() ?? {});
    } catch (e) {
  ...
  Future<void> saveLoyaltyProfile(LoyaltyProfile profile) async {
    try {
      await _firebaseService.firestore.collection('loyalty_profiles').doc(profile.userId).set(profile.toMap());
    } catch (e) {
  ...
  Future<List<Reward>> getAvailableRewards() async {
    try {
      final snapshot = await _firebaseService.firestore.collection('rewards').get();
      return snapshot.docs.map((doc) => Reward.fromMap(doc.data())).where((reward) => reward.isAvailable).toList();
    } catch (e) {
  ...
  Future<List<PointsRule>> getPointsRules() async {
    try {
      final snapshot = await _firebaseService.firestore.collection('points_rules').get();
      return snapshot.docs.map((doc) => PointsRule.fromMap(doc.data())).toList();
    } catch (e) {
      return [];
    }
  }
  ...
  Future<List<LoyaltyAchievement>> getAchievements() async {
    try {
      final snapshot = await _firebaseService.firestore.collection('achievements').get();
      return snapshot.docs.map((doc) => LoyaltyAchievement.fromMap(doc.data())).toList();
    } catch (e) {
      return [];
    }
  }
  ...
  Future<void> createReferral({
    ...
    try {
      ...
      await _firebaseService.firestore.collection('referrals').doc(referralId).set(referral);
    } catch (e) {
  ...
  Future<List<ReferralRecord>> getReferralRecords(String userId) async {
    try {
      final snapshot = await _firebaseService.firestore.collection('referrals').where('referrerId', isEqualTo: userId).get();
      return snapshot.docs.map((doc) => ReferralRecord.fromMap(doc.data())).toList();
    } catch (e) {
      return [];
    }
  }
